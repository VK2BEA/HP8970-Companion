           ...
           tGlobal *          pGlobal)
{
    ...
    extern cairo_user_data_key_t hardcopyKey;
    cairo_set_user_data ( cr, &hardcopyKey, (void *)TRUE, NULL );
...
CB_btn_Print ( GtkButton* wBtnOptions, gpointer user_data ) {
    ...
    res = gtk_print_operation_run (printOp, GTK_PRINT_OPERATION_ACTION_PRINT_DIALOG,
              GTK_WINDOW(g_hash_table_lookup ( pGlobal->widgetHashTable, (gconstpointer)"WID_HP8970_application") ), NULL);
