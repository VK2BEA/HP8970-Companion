 ...
 */
void
leftJustifiedCairoText(cairo_t *cr, gchar *sLabel, gdouble x, gdouble y, gboolean bMask)
{
    cairo_text_extents_t extents;
    GdkRGBA white = { 1.0, 1.0, 1.0, 0.7 };
    cairo_move_to(cr, x, y );
    cairo_text_extents (cr, sLabel, &extents);
    gdouble margin = extents.height * 0.1;
    if( bMask ) {
        cairo_save( cr );
        gdk_cairo_set_source_rgba (cr, &white );
        cairo_rel_move_to( cr, extents.x_bearing, extents.y_bearing - margin );
        cairo_rel_line_to( cr, extents.width, 0.0 );
        cairo_rel_line_to( cr, 0.0, extents.height + 2 * margin );
        cairo_rel_line_to( cr, -extents.width, 0.0 );
        cairo_close_path ( cr );
        cairo_fill( cr );
        cairo_restore( cr );
 ...
 */
void
rightJustifiedCairoText(cairo_t *cr, gchar *sLabel, gdouble x, gdouble y, gboolean bMask )
{
    cairo_text_extents_t extents;
    gdouble justifiedX, justifiedY;
    GdkRGBA white = { 1.0, 1.0, 1.0, 0.7 };
    cairo_move_to(cr, x - stringWidthCairoText(cr, sLabel), y );
    cairo_text_extents (cr, sLabel, &extents);
    gdouble margin = extents.height * 0.1;
    if( bMask ) {
        cairo_get_current_point( cr, &justifiedX, &justifiedY );
        cairo_save( cr );
        gdk_cairo_set_source_rgba (cr, &white );
        cairo_rel_move_to( cr, extents.x_bearing, extents.y_bearing - margin );
        cairo_rel_line_to( cr, extents.width, 0.0 );
        ...
        cairo_move_to ( cr, justifiedX, justifiedY );
    }
    cairo_show_text (cr, sLabel);
}
 ...
 */
void
centreJustifiedCairoText(cairo_t *cr, gchar *sLabel, gdouble x, gdouble y, gdouble backgroundOpacity)
{
    cairo_text_extents_t extents;
    gdouble justifiedX, justifiedY;
    GdkRGBA white = { 1.0, 1.0, 1.0, backgroundOpacity };
    ...
    if( backgroundOpacity != 0.0  ) {
        ...
        cairo_save( cr );
        gdk_cairo_set_source_rgba (cr, &white );
        cairo_rel_move_to( cr, extents.x_bearing - margin, extents.y_bearing - margin );
        cairo_rel_line_to( cr, extents.width + 2 * margin, 0.0 );
        cairo_rel_line_to( cr, 0.0, extents.height + 2 * margin );
        cairo_rel_line_to( cr, -(extents.width + 2 * margin), 0.0 );
        cairo_close_path ( cr );
        cairo_fill( cr );
...
cairo_user_data_key_t hardcopyKey;

void
drawHPlogo (cairo_t *cr, gdouble centreX, gdouble lowerLeftY, gdouble scale, gint bbVariant )
{
    gboolean bHardcopy = GPOINTER_TO_INT( cairo_get_user_data (cr, &hardcopyKey) );
    static gchar *sVariants[] = { "HP 8970A", "HP 8970B", "HP 8970B / 20" };
...
#define HP_LOGO_HEIGHT  (DECAL_WIDTH * 0.08185185)
	cairo_save( cr ); {
        ...
        rightJustifiedCairoText( cr, sVariants[ bbVariant ], DECAL_WIDTH, -14.0, FALSE);
        rightJustifiedCairoText( cr, "Noise Figure Meter", DECAL_WIDTH, 0.0, FALSE );
	...
	/*
    cairo_surface_t *image = cairo_image_surface_create_from_png ("mode-1.0.png");
    cairo_scale( cr, 1.5 * scale, 1.5 * scale );
...
static struct _RsvgHandle *SVGhandles[ eMode_Max ];

void
freeSVGhandles( void ) {
    g_object_unref( SVGhandles[ eMode1_0 ] );
    g_object_unref( SVGhandles[ eMode1_1 ] );
    g_object_unref( SVGhandles[ eMode1_3 ] );
}

void
drawModeDiagram( cairo_t *cr, tMode mode, gint HP8970model, gdouble areaWidth, gdouble areaHeight, gdouble scale ) {

    gdouble width  = areaWidth  * scale;
    gdouble height = areaHeight * scale;

    static gboolean bFirst = TRUE;

    if( bFirst ) {
        SVGhandles[ eMode1_0 ] = rsvg_handle_new_from_data( SVGdiagram_Mode0, sizeof( SVGdiagram_Mode0 ) / sizeof( guint8 ) - 1, NULL );
        SVGhandles[ eMode1_1 ] = rsvg_handle_new_from_data( SVGdiagram_Mode1, sizeof( SVGdiagram_Mode1 ) / sizeof( guint8 ) - 1, NULL );
        SVGhandles[ eMode1_2 ] = SVGhandles[ eMode1_1 ];
        SVGhandles[ eMode1_3 ] = rsvg_handle_new_from_data( SVGdiagram_Mode3, sizeof( SVGdiagram_Mode3 ) / sizeof( guint8 ) - 1, NULL );
        SVGhandles[ eMode1_4 ] = SVGhandles[ eMode1_3 ];
        bFirst = FALSE;
    }

    static gchar *annotationMode1_0[] = {
            "10 — 1500 MHz measurement, no external LO or frequency conversion.",
            "10 — 1600 MHz measurement, no external LO or frequency conversion.",
            "10 — 2047 MHz measurement, no external LO or frequency conversion."
    };

    static gchar *annotationsLOmodes[][3] = {
            { "Mode 1.1", "Variable-frequency external local oscillator, fixed IF,",
                    "frequency conversion in measurement system but not in DUT." },
            { "Mode 1.2", "Fixed-frequency external local oscillator, variable IF, single sideband,",
                    "frequency conversion in measurement system but not in DUT." },
            { "Mode 1.3", "Variable-frequency external local oscillator, fixed IF,",
                    "frequency conversion in DUT (for testing mixer or receiver)." },
            { "Mode 1.4", "Fixed-frequency external local oscillator, variable IF,",
                    "frequency conversion in DUT (for testing mixer or receiver)."}
    };

    gdouble fontSize = (gdouble)areaWidth / 40.0;

    cairo_save( cr );

//  RsvgHandle *handle = rsvg_handle_new_from_file("mode-1.0.opt.svg", 0);
    RsvgRectangle viewport = {
        .x = (areaWidth - width) / 2.0,
        .y = 0,
        .width =  width,
        .height = height,
      };

    rsvg_handle_render_document ( SVGhandles[ mode ] , cr, &viewport, NULL );
    // g_object_unref(handle);

    cairo_select_font_face(cr, MODE_DIAGRAM_FONT, CAIRO_FONT_SLANT_NORMAL, CAIRO_FONT_WEIGHT_BOLD);
    cairo_set_font_size( cr, fontSize );
    leftJustifiedCairoText( cr, mode == eMode1_0 ? "Mode 1.0" : annotationsLOmodes[mode - 1][0], areaWidth * 0.15, areaHeight * 0.10, FALSE );
    cairo_select_font_face(cr, MODE_DIAGRAM_FONT, CAIRO_FONT_SLANT_ITALIC, CAIRO_FONT_WEIGHT_NORMAL);
    cairo_set_font_size( cr, fontSize * 0.80 );

#define HP8970A_INPUT_RANGE "10 — 1500 MHz"
#define HP8970B_INPUT_RANGE "10 — 1600 MHz"
#define HP8970Bopt20_INPUT_RANGE "10 — 2047 MHz"
    gchar *sVariantRanges[] = { HP8970A_INPUT_RANGE, HP8970B_INPUT_RANGE, HP8970Bopt20_INPUT_RANGE };
#define HP8970_INPUT_RANGE_XPOS (areaWidth * 0.70)
#define HP8970_INPUT_RANGE_YPOS_MODE1_0 (areaHeight * 0.282)
#define HP8970_INPUT_RANGE_YPOS_MODEnot1_0 (areaHeight * 0.14)

    if( mode == eMode1_0 ) {
        centreJustifiedCairoText( cr, annotationMode1_0[ HP8970model ], areaWidth / 2.0, areaHeight * 0.59, FALSE );
    } else {
        leftJustifiedCairoText ( cr, annotationsLOmodes[mode - 1][1], areaWidth * 0.15, areaHeight * 0.690, FALSE );
        rightJustifiedCairoText( cr, annotationsLOmodes[mode - 1][2], areaWidth * 0.85, areaHeight * 0.725, FALSE );
    }

    fontSize = (gdouble)areaWidth / 57.0;
    cairo_select_font_face(cr, MODE_DIAGRAM_FONT_CONDENSED, CAIRO_FONT_SLANT_NORMAL, CAIRO_FONT_WEIGHT_NORMAL);
    cairo_set_font_size( cr, fontSize );
    if( mode == eMode1_0 ) {
        leftJustifiedCairoText( cr, sVariantRanges[ HP8970model ],
                                HP8970_INPUT_RANGE_XPOS, HP8970_INPUT_RANGE_YPOS_MODE1_0, FALSE );
    } else {
        leftJustifiedCairoText( cr, sVariantRanges[ HP8970model ],
                                HP8970_INPUT_RANGE_XPOS, HP8970_INPUT_RANGE_YPOS_MODEnot1_0, FALSE );
    }
    cairo_restore( cr );
}
