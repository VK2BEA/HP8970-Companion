...
// settings
        ...
        if( json_reader_read_member (reader, "noisePlotAutoscale") == TRUE )
            pGlobal->flags.bAutoScaling = json_reader_get_boolean_value ( reader );
        json_reader_end_member (reader);

        if( !pGlobal->flags.bAutoScaling ) {
            if( json_reader_read_member (reader, "noisePlotScaleMin") == TRUE )
                pGlobal->fixedGridNoise[ pGlobal->plot.noiseUnits ][ 0 ] = json_reader_get_double_value ( reader );
            json_reader_end_member (reader);
            if( json_reader_read_member (reader, "noisePlotScaleMax") == TRUE )
                pGlobal->fixedGridNoise[ pGlobal->plot.noiseUnits ][ 1 ] = json_reader_get_double_value ( reader );
            json_reader_end_member (reader);
            if( json_reader_read_member (reader, "gainPlotScaleMin") == TRUE )
                pGlobal->fixedGridGain[ 0 ] = json_reader_get_double_value ( reader );
            json_reader_end_member (reader);
            if( json_reader_read_member (reader, "gainPlotScaleMax") == TRUE )
                pGlobal->fixedGridGain[ 1 ] = json_reader_get_double_value ( reader );
            json_reader_end_member (reader);
        }
...
savePlot( gchar *filePath, tGlobal *pGlobal ) {
    ...
    json_builder_add_string_value ( builder, VERSION );
        ...
        json_builder_begin_object( builder );   // begin settings
            ...
            json_builder_add_boolean_value ( builder, pGlobal->flags.bAutoScaling );
            json_builder_set_member_name (builder, "noisePlotScaleMin");
            json_builder_add_double_value ( builder, pGlobal->fixedGridNoise[ pGlobal->plot.noiseUnits ][ 0 ] );
            json_builder_set_member_name (builder, "noisePlotScaleMax");
            json_builder_add_double_value ( builder, pGlobal->fixedGridNoise[ pGlobal->plot.noiseUnits ][ 1 ] );
            json_builder_set_member_name (builder, "gainPlotScaleMin");
            json_builder_add_double_value ( builder, pGlobal->fixedGridGain[ 0 ] );
            json_builder_set_member_name (builder, "gainPlotScaleMax");
            json_builder_add_double_value ( builder, pGlobal->fixedGridGain[ 1 ] );
...
CB_btn_RestoreJSON ( GtkButton* wBtnRecallHPGL, gpointer user_data ) {
    ...
    gtk_file_filter_add_pattern (filter, "*.[Jj][Ss][Oo][Nn]");
    gtk_file_filter_set_name (filter, "JSON");
    sSuggestedFilename = g_date_time_format( now, pGlobal->flags.bbHP8970Bmodel ?
            "HP8970B.%d%b%y.%H%M%S.json" : "HP8970A.%d%b%y.%H%M%S.json");
    g_list_store_append ( (GListStore*)filters, filter);
